...
//var uriPath = '/perlite/'
//var uriPath = '/' // 注释掉，使用从index.php传递的值

// 全局保护：确保uriPath始终有正确的值
$(document).ready(function() {
  if (typeof uriPath === 'undefined' || uriPath === '/' || uriPath === '') {
    uriPath = '/myvault/';
    console.log('全局uriPath修复:', uriPath);
  }
});
...
function getContent(str, home = false, popHover = false, anchor = "") {
  ...
  // 修复uriPath变量，确保它包含正确的路径
  if (typeof uriPath === 'undefined' || uriPath === '/' || uriPath === '') {
    uriPath = '/myvault/';
    console.log('uriPath被重置为:', uriPath);
  }
  ...
  if (str.length == 0) {
    ...
    console.log('getContent调用，uriPath:', uriPath, 'str:', str, 'requestPath:', requestPath);
    ...
    if (home) {
      ...
      console.log('home模式，requestPath:', requestPath);
    }
